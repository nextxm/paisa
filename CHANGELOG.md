...
- **QueryBuilder component** — New `src/lib/components/QueryBuilder.svelte` provides a visual, dropdown-based filter builder that renders active filters as removable chips. Supports four filter types (`account`, `amount`, `date`, `tag`) each with appropriate operators (equals, not-equals, contains, starts-with, comparison, is-set, etc.). Multiple filters are combined with a togglable AND/OR logic operator displayed as a clickable chip between adjacent filters. The component exports a reactive `predicate: TransactionPredicate` binding that parent pages can subscribe to for real-time filtering. Filter logic is isolated in a companion `query_builder_utils.ts` module with full unit test coverage.
