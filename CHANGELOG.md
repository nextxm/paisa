...
- **GET /api/jobs/:id** — New read endpoint that returns the current state of a background job as a JSON object (fields: `id`, `status`, `created_at`, `started_at`, `finished_at`, `error`). Returns `404` with the standard error envelope when the job ID is unknown.
