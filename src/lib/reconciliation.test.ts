...
import { reconciliationLabel, reconciliationTagClass } from "./reconciliation";
...
describe("reconciliation helpers", () => {
  ...
  test("formats labels for never/today/plural-day states", () => {
    ...
    expect(reconciliationLabel(status({ days_since: 0 }))).toBe("Last reconciled: today");
    expect(reconciliationLabel(status({ days_since: 5 }))).toBe("Last reconciled: 5 days ago");
  });
});
