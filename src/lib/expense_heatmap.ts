...
export function buildHeatmapWeeksOfMonth(
  days: DailyExpenseDay[],
  from: dayjs.Dayjs,
  to: dayjs.Dayjs,
  category = ""
  ...
  const lookup = buildDailyLookup(days, category);
  const startMonth = from.startOf("month");
  const endMonth = to.startOf("month");

  const columns: ExpenseHeatmapCell[][] = [];

  for (
    let currentMonth = startMonth;
    !currentMonth.isAfter(endMonth, "month");
    currentMonth = currentMonth.add(1, "month")
  ) {
    const cells: ExpenseHeatmapCell[] = [];
    ...
    const weekDefinitions = [
      ...
      { start: 15, end: 21 },
      { start: 22, end: 28 },
      { start: 29, end: currentMonth.endOf("month").date() }
    ];

    for (let w = 0; w < 5; w++) {
      const def = weekDefinitions[w];
      const startDay = def.start;
      const endDay = def.end;

      if (startDay > endDay) {
        // e.g. Feb in non-leap year (end is 28, so 29 > 28)
        cells.push({
          date: currentMonth.date(28),
          total: 0,
          inRange: false,
          tooltipRange: ""
        });
      ...
      for (let d = startDay; d <= endDay; d++) {
        const cursor = currentMonth.date(d);
        const key = cursor.format("YYYY-MM-DD");
        weekTotal += lookup.get(key) || 0;

        if (!cursor.isBefore(from, "day") && !cursor.isAfter(to, "day")) {
          hasAnyInRange = true;
        }
      }

      const weekStartDate = currentMonth.date(startDay);
      const weekEndDate = currentMonth.date(endDay);

      cells.push({
        date: weekStartDate,
        total: weekTotal,
        inRange: hasAnyInRange,
        tooltipRange: `${weekStartDate.format("MMM D")} - ${weekEndDate.format("D, YYYY")}`
      });
    }
