...
func (q *Query) All() []posting.Posting {
	...
	result := q.context.Order("date " + q.order + ", amount desc, account asc, id asc").Find(&postings)
	if result.Error != nil {
...
func (q *Query) First() *posting.Posting {
	...
	q.context = q.context.Where("forecast = ?", q.includeForecast)
	result := q.context.Order("date " + q.order + ", amount desc, account asc, id asc").First(&posting)
